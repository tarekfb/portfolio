...
const generateRandomBlobPath = () => {
    return blobs2.svgPath({
        ...config,
        seed: Math.random(), // 🚨 Math.random() is now here
    });
};
...
// need to generate pathadat her ein svg
// update the state
// have the value be the state
// then import this state to whoever needs to consume state
// also export a function that needs to update the state here
// https://www.w3schools.com/react/react_usecontext.asp
// https://nextjs.org/docs/app/getting-started/server-and-client-components#using-context-providers
...
export default function BlobProvider({
    ...
    // Function to update the blob shape to a new random one
    ...
    // The value object to be passed down to consumers
    // useMemo is used to memoize the value object itself, ensuring stability
    const contextValue = useMemo(() => ({
        blobPathData, // The 'get' part (the current state)
        setRandomBlob, // The 'set' part (the updater function)
